...
public enum TaskTriggerRule {
    ...
    ALL_SUCCESS,                 //（默认）仅当所有上游任务都成功时，任务才会运行。
    ...
    ONE_SUCCESS,                 // 至少有一个上游任务已成功（不等待所有上游任务完成）。
    ...
    ONE_DONE,                    // 当至少一个上游任务成功或失败时，任务将运行。
    ...
    NONE_FAILED,                 // 仅当所有上游任务都成功或被跳过时，任务才会运行。
                                 // none_failed 触发器规则仅在所有上游任务成功或跳过时才运行任务。

    NONE_FAILED_MIN_ONE_SUCCESS, // 仅当所有上游任务均未失败，且至少有一个上游任务成功时，任务才会运行。
    ...
    public boolean evaluate(Collection<TaskState> upstreamStates) {
        switch (this) {
            ...
            case ALL_DONE:
                return upstreamStates.stream().noneMatch(s -> s == TaskState.PENDING || s == TaskState.RUNNING);
            case ALL_SKIPPED:
